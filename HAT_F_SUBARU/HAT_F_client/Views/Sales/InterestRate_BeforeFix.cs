...
namespace HatFClient.Views.Sales
{
    ...
    public partial class InterestRate_BeforeFix : Form
    {
        /// <summary>検索条件</summary>
        private List<FilterCriteria> _filters = new();

        /// <summary>検索画面のドロップダウンを定義</summary>
        private readonly List<ColumnMappingConfig> _criteriaDefinition = CriteriaHelper.CreateCriteriaDefinitions<ViewInterestRateBeforeFixSearchCondition>();

        /// <summary>直前で選択されていた売上グリッドの行</summary>
        private int _prevSaleRow = -1;

        /// <summary>売上一覧グリッドにバインドされるデータソース</summary>
        private SortableBindingList<ViewInterestRateBeforeFix> _salesSource;

        /// <summary>明細グリッドにバインドされるデータソース</summary>
        private SortableBindingList<CheckableInterestRateBeforeFix> _detailSource;

        /// <summary>
        /// <para><see cref="ViewInterestRateBeforeFix"/>から<see cref="ViewInterestRateBeforeFix"/>へ変換するためのオブジェクト</para>
        /// <para>売上一覧グリッドに表示する情報を生成するのに使用する</para>
        /// </summary>
        private readonly Mapper _interestRateMapper;

        /// <summary>
        /// <para><see cref="ViewInterestRateBeforeFix"/>から<see cref="CheckableInterestRateBeforeFix"/>へ変換するためのオブジェクト</para>
        /// <para>明細グリッドに表示する情報を生成するのに使用する</para>
        /// </summary>
        private readonly Mapper _detailMapper;

        /// <summary>コンストラクタ</summary>
        public InterestRate_BeforeFix()
        {
            InitializeComponent();
            ...
            if (!this.DesignMode)
            {
                ...
                _interestRateMapper = new(new MapperConfiguration(cfg =>
                {
                    cfg.CreateMap<ViewInterestRateBeforeFix, ViewInterestRateBeforeFix>()
                        .ForMember(x => x.伝票区分名, x => x.MapFrom(src => HatFComParts.FormatOrDefault(src.伝票区分, s => $"{src.伝票区分}：{src.伝票区分名}", string.Empty)));
                }));
                _detailMapper = new(new MapperConfiguration(cfg => cfg.CreateMap<ViewInterestRateBeforeFix, CheckableInterestRateBeforeFix>()));
            }
        }
        ...
        private async void InterestRate_BeforeFix_Load(object sender, EventArgs e)
        {
            UpdateEnabled();
        ...
        private async Task SyncDataAsync(List<FilterCriteria> criterias)
        {
            ...
            {
                return SearchRepo.GetInstance().SearchInterestRateBeforeFixesAsync(
        ...
        private async void BtnSearch_Click(object sender, EventArgs e)
        {
            ...
            {
                var dropDownInfo = InitializeDropDownInfo().ToList();
                searchFrm.AddDropDownItems(dropDownInfo);
                ...
                {
                    txtCondition.Text = FilterHelper.CreateFilterOptionStr(
                        searchFrm.FilterCriterias.Select(f => f.AsFilterOptionAndCaption()).ToList(), dropDownInfo);
        ...
        private void BindSalesList(List<ViewInterestRateBeforeFix> list)
        {
            ...
            _salesSource = new SortableBindingList<ViewInterestRateBeforeFix>(
                _interestRateMapper.Map<List<ViewInterestRateBeforeFix>>(group.Select(x => x.First())));
            ...
            grdDetail.DataSource = Enumerable.Empty<ViewInterestRateBeforeFix>();
        }
        ...
        private void GrdList_SelChange(object sender, EventArgs e)
        {
            ...
            var selectedItem = grdList.Rows[grdList.Selection.BottomRow].UserData as IGrouping<string, ViewInterestRateBeforeFix>;
            BindDetailList(selectedItem);
        }

        /// <summary>明細にバインドする</summary>
        /// <param name="list">表示データ</param>
        private void BindDetailList(IEnumerable<ViewInterestRateBeforeFix> list)
        {
            _detailSource = new SortableBindingList<CheckableInterestRateBeforeFix>(_detailMapper.Map<List<CheckableInterestRateBeforeFix>>(list));
        ...
        private async void BtnSave_Click(object sender, EventArgs e)
        {
            var checkedInterestRates = _detailSource.Where(x => x.選択)
                .Select(x => new InterestRateCheckBeforeFixParameter()
                {
                    SaveKey = x.SaveKey,
                    DenSort = x.DenSort,
                    DenNoLine = x.DenNoLine,
                    Comment = txtComment.Text,
                });
            var result = await ApiHelper.UpdateAsync(this, () =>
            {
                return Program.HatFApiClient.PutAsync<List<InterestRateCheckFixedResult>>(
                    ApiResources.HatF.Client.InterestRateCheckBeforeFix, checkedInterestRates);
            });
            ...
            foreach (var target in updateTargets)
            {
                ...
                var salesSource = grdList.Rows.OfType<Row>().Skip(1)
                    .Select(x => (x.UserData as IGrouping<string, ViewInterestRateBeforeFix>))
                    .SelectMany(x => x.AsEnumerable())
                    .Where(x => x.SaveKey == target.SaveKey)
                    .Where(x => x.DenSort == target.DenSort)
                    .Where(x => x.DenNoLine == target.DenNoLine)
                    .FirstOrDefault();
