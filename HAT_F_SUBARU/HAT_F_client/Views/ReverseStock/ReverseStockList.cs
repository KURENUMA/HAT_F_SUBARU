...
using HatFClient.Views.ReturnReceiving;
...
{
    public partial class ReverseStockList : Form
    {
        private class ReverseStockGridManager : GridManagerBase<ViewSalesReturn> { }

        private ReverseStockGridManager gridManager = new ReverseStockGridManager();
        //データテーブルレイアウト
        ...
        private static readonly Type TARGET_MODEL = typeof(HAT_F_api.Models.ViewSalesReturn);
        private const string OUTFILE_NAME = "返品入力一覧_yyyyMMdd_HHmmss";

        public ReverseStockList()
        {
            InitializeComponent();
        ...
        private void InitializeFetching()
        {
            ...
            gridManager.FetchFuncAsync = async (filter) => {
                // API(ページング条件付与)
                string url = ApiHelper.AddPagingQuery(ApiResources.HatF.Search.SalesReturn, gridManager.CurrentPage, gridManager.PageSize);
                var conditions = filter.Select(f => f.AsFilterOption()).ToList();

                var apiResponse = await Program.HatFApiClient.PostAsync<List<ViewSalesReturn>>(
                    url,   // 一覧取得API
            ...
            {
                string url = ApiResources.HatF.Search.SalesReturnCount;
                var conditions = filter.Select(f => f.AsFilterOption()).ToList();
        ...
        private void frmRequest_Load(object sender, EventArgs e)
        {
            ...
            gridOrderManager = new GridOrderManager(CriteriaHelper.CreateCriteriaDefinitions<ViewSalesReturn>());
        ...
        {
            using (var searchFrm = new FrmAdvancedSearch(CriteriaHelper.CreateCriteriaDefinitions<ViewSalesReturn>(), gridManager.Filters))
            {
        ...
        private void btnDetail_Click(object sender, EventArgs e)
        {
            ...
            if (int.TryParse(grid.GetData(grid.RowSel, "承認ステータス区分").ToString(), out int value)){
                if(value > 3)
                {
                    SalesRefundDetail SalesRefundDetail = FormFactory.GetModelessForm<SalesRefundDetail>();
                    SalesRefundDetail.Condition.Hat注文番号 = grid.GetData(grid.RowSel, "Hat注文番号").ToString();
                    SalesRefundDetail.Condition.伝票番号 = grid.GetData(grid.RowSel, "伝票番号").ToString();
                    SalesRefundDetail.Show();
                    SalesRefundDetail.Activate();
                    return;
                }
            }
            ReverseStockDetail detail = FormFactory.GetModelessForm<ReverseStockDetail>();
        ...
        private void ReverseStockList_FormClosed(object sender, FormClosedEventArgs e)
        {
            FormFactory.GetModelessFormCache<ReverseStockDetail>()?.Close();
            FormFactory.GetModelessFormCache<SalesRefundDetail>()?.Close();
        }
    }
}
