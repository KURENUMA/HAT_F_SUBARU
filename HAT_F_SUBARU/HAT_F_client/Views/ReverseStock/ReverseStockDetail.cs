...
{
    public partial class ReverseStockDetail : Form
    {
        ...
        public ViewSalesReturnDetail Condition { get; set; } = new ViewSalesReturnDetail();

        public ReverseStockDetail()
        {
            InitializeComponent();
        ...
        {
            var response = await Program.HatFApiClient.GetAsync<List<Employee>>(ApiResources.HatF.MasterEditor.EmloyeeUserAssignedRole, new { userRoleCd = "12" }); //12:返品入力権限
        ...
        private async void ReturnReceivingDetail_Load(object sender, EventArgs e)
        {
            ...
            {
                c1FlexGrid_ReturnReceivingDetail.DataSource = new List<ViewSalesReturnDetail>();
            }
        ...
        private ApprovalRequestEx ToApprovalObjects(ApprovalResult requestType)
        {
            ...
            for (var i = 1; i < c1FlexGrid_ReturnReceivingDetail.Rows.Count; i++)
            {
                ...
                {
                    if (!String.IsNullOrEmpty(r.返品依頼数量?.ToString()))
                    {
...
　　　　　　　　            Quantity = r.現在数量, // 売上数量
        ...
        private void c1FlexGrid_ReturnReceivingDetail_AfterEdit(object sender, RowColEventArgs e)
        {
            ...
            if (c1FlexGrid_ReturnReceivingDetail.Cols[editedCol].Name == "返品依頼数量")
            {
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            {
                if (String.IsNullOrEmpty(c1FlexGrid_ReturnReceivingDetail[row, "返品依頼数量"]?.ToString()))
                {
                    //色を戻す
                    c1FlexGrid_ReturnReceivingDetail.GetCellRange(row, c1FlexGrid_ReturnReceivingDetail.Cols["返品依頼数量"].Index).StyleNew.BackColor = SystemColors.Window;
                }
                else
                {
                    c1FlexGrid_ReturnReceivingDetail.GetCellRange(row, c1FlexGrid_ReturnReceivingDetail.Cols["返品依頼数量"].Index).StyleNew.BackColor = Color.Yellow; // 背景色を黄色に変更
                }
            }
        }
