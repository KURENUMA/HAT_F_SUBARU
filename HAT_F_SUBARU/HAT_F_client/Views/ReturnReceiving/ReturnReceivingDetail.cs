...
{
    public partial class ReturnReceivingDetail : Form
    {
        private PurchaseRepo _purchaseRepo;
        private RoleController _roleController;
        private readonly string approvalType = "R5"; //U1:仕入売上訂正、R1:返品入力、R5:返品入庫 
        ...
        public ViewSalesReturnReceiptDetail Condition { get; set; } = new ViewSalesReturnReceiptDetail();

        public ReturnReceivingDetail()
        {
            InitializeComponent();
        ...
        {
            var response = await Program.HatFApiClient.GetAsync<List<Employee>>(ApiResources.HatF.MasterEditor.EmloyeeUserAssignedRole, new { userRoleCd = "13" }); //13:返品商品の検品承認権限
        ...
        private async Task SetApprovalDataAsync()
        {
            ...
            for (var i = 1; i < c1FlexGrid_ReturnReceivingDetail.Rows.Count; i++)
            {
                ...
                var r = c1FlexGrid_ReturnReceivingDetail.Rows[i].DataSource as ViewSalesReturnReceiptDetail;
                if (!String.IsNullOrEmpty(r.入庫承認要求番号?.ToString()))
                {
                    approvalReqNo = r.入庫承認要求番号;
                }
            }
        ...
        private async void ReturnReceivingDetail_Load(object sender, EventArgs e)
        {
            ...
            {
                c1FlexGrid_ReturnReceivingDetail.DataSource = new List<ViewSalesReturnReceiptDetail>();
            }
        ...
        private ApprovalRequestEx ToApprovalObjects(ApprovalResult requestType)
        {
            ...
            for (var i = 1; i < c1FlexGrid_ReturnReceivingDetail.Rows.Count; i++)
            {
                ...
                if (!String.IsNullOrEmpty(r.入庫承認要求番号?.ToString()))
                {
                    approvalEx.approvalReqNo = r.入庫承認要求番号;
                }
                if (requestType == ApprovalResult.Request)
                {
                    if (!String.IsNullOrEmpty(r.返品数量?.ToString()))
                    {
                        target.Add(new SalesReturn()
                        {
                            ReturningProductsId = r.返品id, // 返品ID
                            RowNo = r.返品行番号, // 行番号
                            ReturnQuantity = r.返品数量 //返品数量
                        });
                    }
                }
            }
        ...
        private void c1FlexGrid_ReturnReceivingDetail_AfterEdit(object sender, RowColEventArgs e)
        {
            ...
            if (c1FlexGrid_ReturnReceivingDetail.Cols[editedCol].Name == "返品数量")
            {
                // 数量と単価が入力されているセルの値を取得
                object returnQuantity = c1FlexGrid_ReturnReceivingDetail.GetDataDisplay(editedRow, c1FlexGrid_ReturnReceivingDetail.Cols["返品数量"].Index);
                ...
                {
                    c1FlexGrid_ReturnReceivingDetail[editedRow, c1FlexGrid_ReturnReceivingDetail.Cols["返品後数量"].Index] = String.Empty;
                    return;
                }
                // 数量と単価が数値に変換可能かチェック
                if (decimal.TryParse(returnQuantity?.ToString(), out decimal returnQ) &&
                    decimal.TryParse(Quantity?.ToString(), out decimal quantity))
                {
                    // 合計金額を計算
                    decimal totalQuantity = quantity - returnQ;

                    // 合計金額を別のセルに出力
                    c1FlexGrid_ReturnReceivingDetail[editedRow, c1FlexGrid_ReturnReceivingDetail.Cols["返品後数量"].Index] = totalQuantity;
                }
            }
        }
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            {
                if (String.IsNullOrEmpty(c1FlexGrid_ReturnReceivingDetail[row, "返品数量"]?.ToString()))
                {
                    //色を戻す
                    c1FlexGrid_ReturnReceivingDetail.GetCellRange(row, c1FlexGrid_ReturnReceivingDetail.Cols["返品数量"].Index).StyleNew.BackColor = SystemColors.Window;
                }
                else
                {
                    c1FlexGrid_ReturnReceivingDetail.GetCellRange(row, c1FlexGrid_ReturnReceivingDetail.Cols["返品数量"].Index).StyleNew.BackColor = Color.Yellow; // 背景色を黄色に変更
                }
            }
        }
