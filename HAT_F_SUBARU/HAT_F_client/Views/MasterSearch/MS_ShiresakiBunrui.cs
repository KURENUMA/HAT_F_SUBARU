...
namespace HatFClient.Views.MasterSearch
{
    public partial class MS_ShiresakiBunrui : Form
    {
        ...
        private string _strMsShiresakiBunruiCode = @"";
        private string _strMsShiresakiBunruiName = @"";
        ...
        public string StrMsShiresakiBunruiCode
        {
            get { return _strMsShiresakiBunruiCode; }
        }
        public string StrMsShiresakiBunruiName
        {
            get { return _strMsShiresakiBunruiName; }
        }
        ...
        private async void ShowList()
        {
            ...
            var supplierCategories = await ApiHelper.FetchAsync(this, () =>
            {
                return repo.searchSupplierCategory(shiresakiCode, shiresakiName, shiresakiKana, teamCode, IntShowMaxConunt);
            });
            if(supplierCategories.Failed)
            {
                return;
            }

            dgvList.Rows.Clear();
            if (supplierCategories.Value.Count > 0)
            {
                for (int i = 0; i < supplierCategories.Value.Count; i++)
                {
                    var values = new string[] { 
                        supplierCategories.Value[i].SupplierCd, 
                        supplierCategories.Value[i].SupplierName, 
                        supplierCategories.Value[i].CategoryCode, 
                        supplierCategories.Value[i].CategoryName, 
                        supplierCategories.Value[i].SupplierFax, 
                        supplierCategories.Value[i].SupplierTel,
                    };
        ...
        private void BtnFnc11_Click(object sender, System.EventArgs e)
        {
            ...
            if (dgvList.CurrentRow != null)
            {
                ...
                _strMsShiresakiBunruiCode = dgvList.Rows[dgvList.CurrentRow.Index].Cells[2].Value.ToString();
                _strMsShiresakiBunruiName = dgvList.Rows[dgvList.CurrentRow.Index].Cells[3].Value.ToString();
            }
        }
