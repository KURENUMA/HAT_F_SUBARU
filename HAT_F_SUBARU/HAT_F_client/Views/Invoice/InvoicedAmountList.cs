...
{
    public partial class InvoicedAmountList : Form
    {
        // TODO: 画面固有のvGridManagervを定義
        private class InvoicedAmountListGridManager : GridManagerBase<ViewInvoicedAmount> { }

        // TODO: TemplateGridManager から画面用の固有型へ変更 (GridManagerBase<T>の継承で作れます)
        private InvoicedAmountListGridManager gridManager = new InvoicedAmountListGridManager();
        ...
        private static readonly Type TARGET_MODEL = typeof(HAT_F_api.Models.ViewInvoicedAmount);
        private const string OUTFILE_NAME = "請求済一覧_yyyyMMdd_HHmmss";

        public InvoicedAmountList()
        {
            InitializeComponent();           
        ...
        private void InitializeFetching()
        {
            ...
            gridManager.FetchFuncAsync = async (filter) => {
                // API(ページング条件付与)
                string url = ApiHelper.AddPagingQuery(ApiResources.HatF.Search.InvoiceAmountList, gridManager.CurrentPage, gridManager.PageSize);
                var conditions = filter.Select(f => f.AsFilterOption()).ToList();

                var apiResponse = await Program.HatFApiClient.PostAsync<List<ViewInvoicedAmount>>(
                    url,   // 一覧取得API
            ...
            {
                string url = ApiResources.HatF.Search.InvoiceAmountListCount;
                var conditions = filter.Select(f => f.AsFilterOption()).ToList();
        ...
        {
            // TODO:引数を変更
            gridOrderManager = new GridOrderManager(CriteriaHelper.CreateCriteriaDefinitions<ViewInvoicedAmount>());
        ...
        {
            using (var searchFrm = new FrmAdvancedSearch(CriteriaHelper.CreateCriteriaDefinitions<ViewInvoicedAmount>()))
            {
        ...
        private async void btnDetail_Click(object sender, EventArgs e)
        {
            ...
            string companyCode = grid.GetData(grid.RowSel, "得意先コード").ToString();

            var detail = await ApiHelper.FetchAsync(this, () =>
                Program.HatFApiClient.GetAsync<List<ViewInvoiceDetail>>(ApiResources.HatF.Client.InvoiceDetail, new { companyCode })
            );
            if (detail.Failed)
            {
                return;
            ...
            var InvoicedAmountDetail = new InvoicedAmountDetail();
            InvoicedAmountDetail.LoadDataAndShow(detail.Value);
        }
    }
}
