...
namespace HatFClient.Views.Invoice
{
    public partial class InvoiceList : Form
    {
        ...
        private class InvoiceListGridManager : GridManagerBase<ViewInvoice> { }

        // TODO: TemplateGridManager から画面用の固有型へ変更 (GridManagerBase<T>の継承で作れます)
        private InvoiceListGridManager gridManager = new InvoiceListGridManager();
        ...
        private static readonly Type TARGET_MODEL = typeof(HAT_F_api.Models.ViewInvoice);
        private const string OUTFILE_NAME = "請求一覧_yyyyMMdd_HHmmss";
        ...
        private void InitializeFetching()
        {
            ...
            gridManager.FetchFuncAsync = async (filter) => {
                // API(ページング条件付与)
                string url = ApiHelper.AddPagingQuery(ApiResources.HatF.Search.InvoiceList, gridManager.CurrentPage, gridManager.PageSize);
                var conditions = filter.Select(f => f.AsFilterOption()).ToList();

                var apiResponse = await Program.HatFApiClient.PostAsync<List<ViewInvoice>>(
                    url,   // 一覧取得API
            ...
            {
                string url = ApiResources.HatF.Search.InvoiceListCount;
                var conditions = filter.Select(f => f.AsFilterOption()).ToList();
        ...
        {
            // TODO:引数を変更
            gridOrderManager = new GridOrderManager(CriteriaHelper.CreateCriteriaDefinitions<ViewInvoice>());
        ...
        {
            using (var searchFrm = new FrmAdvancedSearch(CriteriaHelper.CreateCriteriaDefinitions<ViewInvoice>()))
            {
        ...
        private async void btnDetail_ClickAsync(object sender, EventArgs e)
        {
            ...
            InvoiceDetail.LoadDataAndShow(detail.Value);
        }
    }
}
