...
{
    public partial class WH_Receivings : Form
    {
        // TODO: 画面固有のvGridManagervを定義
        private class WHReceivingsGridManager : GridManagerBase<ViewWarehousingReceiving> { }
        ...
        private static readonly Type TARGET_MODEL = typeof(HAT_F_api.Models.ViewWarehousingReceiving);
        private const string OUTFILE_NAME = "入庫予定一覧_yyyyMMdd_HHmmss";

        public WH_Receivings()
        {
            InitializeComponent();           
        ...
        private void InitializeFetching()
        {
            ...
            gridManager.FetchFuncAsync = async (filter) => {
                // API(ページング条件付与)
                string url = ApiHelper.AddPagingQuery(ApiResources.HatF.Client.WarehousingReceivings2, gridManager.CurrentPage, gridManager.PageSize);
                var conditions = filter.Select(f => f.AsFilterOption()).ToList();

                var apiResponse = await Program.HatFApiClient.PostAsync<List<ViewWarehousingReceiving>>(
                    url,   // 一覧取得API
            ...
            {
                string url = ApiResources.HatF.Client.WarehousingReceivingsCount2;
                var conditions = filter.Select(f => f.AsFilterOption()).ToList();
        ...
        {
            // TODO:引数を変更
            gridOrderManager = new GridOrderManager(CriteriaHelper.CreateCriteriaDefinitions<ViewWarehousingReceiving>());
        ...
        {
            var criteriaDefinition = CriteriaHelper.CreateCriteriaDefinitions<ViewWarehousingReceiving>();
