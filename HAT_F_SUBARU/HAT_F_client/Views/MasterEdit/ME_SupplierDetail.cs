...
namespace HatFClient.Views.MasterEdit
{
    ...
    public partial class ME_SupplierDetail : Form
    {
        ...
        private async void ME_SupplierDetail_Load(object sender, EventArgs e)
        {
            ...
            if (IsUpdateMode)
            {
                var url = string.Format(ApiResources.HatF.MasterEditor.Supplier, _supCode, 0);
        ...
        /// <returns>仕入先情報</returns>
        private async Task<SupplierMst> GetSupplierAsync(string supCode,short supSubNo)
        {
            var url = string.Format(ApiResources.HatF.MasterEditor.Supplier, supCode, supSubNo);
            var supplier = await ApiHelper.FetchAsync(this, async () =>
            {
                return await Program.HatFApiClient.GetAsync<SupplierMst>(url);
            });
