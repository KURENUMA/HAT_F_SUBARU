...
{
    public abstract class GridManagerBase<T> : SourceConvertingGridManagerBase<T, T> where T : class { }

    public abstract class SourceConvertingGridManagerBase<T, TSource> : IGridManager where T : class
    {
        ...
        public List<SearchDropDownInfo> DropDownItems { get; set; } = null;
        ...
        public async Task<bool> Reload(List<FilterCriteria> filters)
        {
            ...
            //検索条件を表示用文字列に変換
            FilterOptionStr = FilterHelper.CreateFilterOptionStr(
                filters.Select(f => f.AsFilterOptionAndCaption()).ToList(), this.DropDownItems);
